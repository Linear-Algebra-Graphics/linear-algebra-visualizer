...
function leftMostNonZeroInRow(matrix, row) {
 ...
 //THIS ONE IS THE ONE THAT WORKS, REMOVE DEMO PARTS AND IN CODE COMMENt FOR PLANE SIZE TO RETURN TO NEWEST WORKING ONE
 class GaussianPlanes {
    ...
    constructor(graph, planesStdForm, planeColors) {
        ...
        //used for DEMO
        for (i = 0; i < planeCenter.length; i++) {
            planeCenter[i] = this.solution
        }
        //end used for DEMO
    ...
    draw() {
        ...
        for (let i = 0; i < this.planesStdForm.length; i++) {
            let currPlaneLines = getPlaneLines(this.graph, this.planesStdForm[i], /*5 / this.graph.currentZoom*/ 10, this.planeCenter[this.planeOrderFromInput[i]])
